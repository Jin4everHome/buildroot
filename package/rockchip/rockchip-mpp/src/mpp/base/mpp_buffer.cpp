...
int mpp_buffer_get_ref_count(MppBuffer buffer)
{
    MppBufferImpl *Impl = (MppBufferImpl*)buffer;
    if (NULL == buffer) {
        mpp_err("mpp_buffer_inc_ref invalid input: buffer NULL from\n");
        return MPP_ERR_UNKNOW;
    }

    return (int)Impl->ref_count;
}
...
RK_S32  mpp_buffer_group_used(MppBufferGroup group)
{
    if (NULL == group) {
        mpp_err_f("input invalid group %p\n", group);
        return MPP_NOK;
